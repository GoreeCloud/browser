...
- Current Stable authority revision: `b7fa8164bfdeaa1dc0acb21b770e7601120da04e`
...
- Immediate shared rollback baseline: `1.4.1`
...
The V1.5 migration requires more than replacing a version string. Browser must audit and update, where applicable:
...
16. Browser-local rollback from V1.5 to the immediate V1.4.1 baseline.
...
The omnibox and other Browser-owned search/navigation surfaces are Browser UI and therefore require Browser-local V1.5 acceptance even though GoreeCloud Search or GoreeCloud Index may own underlying query/capability authority.
...
## V1.5.1 central hardening boundary

The central Glaze V1.5.0 release explicitly moved two Glaze-level qualification expansions to V1.5.1:

- `performance-representative-budget`;
- `platform-posture-continuity`.

Their movement is a central design-system release-scope decision, not PASS evidence. It does not remove Browser's own responsibility to validate representative performance, supported form factors, foldable/posture behavior where Browser supports them, and production behavior before Browser promotion.
...
- exact source mapping against Stable authority `b7fa8164bfdeaa1dc0acb21b770e7601120da04e` and reviewed implementation anchor `ee1032a0822ab8e103f8afe48e5c1859fde65cc9`;
...
Browser remains `applicable-migration-required` and nonconformant wherever required V1.5 migration or Browser-specific acceptance evidence is incomplete.
