...
## Synthetic test-fixture finding

The full-history Gitleaks gate has one narrowly classified synthetic test-fixture finding from commit `89b31317cefc7955add9f108e44ff64fa3fd1c6a`: `capabilityTokenReference = "psc_test-capability-123"` in `PrivacyShieldSearchAuthorizationTest.kt`. The value is a test-only placeholder, not a credential. Its exact Gitleaks fingerprint is recorded in the repository-root `.gitleaksignore`; no path-, rule-, or repository-wide bypass is used. Any different finding still fails the gate.
